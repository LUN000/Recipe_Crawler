...
#str轉int
for index in range(len(Icook['Like'])):
    value= Icook['Like'][index]
    #(eval(re.sub(r'萬','',value)))*10000
    Icook['Like'][index] = (eval(re.sub(r'萬','',value)))*10000 if '萬' in value else eval(value)
     
Icook['Time'],Icook['Like'] = Icook['Time'].astype(int), Icook['Like'].astype(int)
...
if not os.path.exists(f"{food}Recipe"):
        os.mkdir(f"{food}Recipe")
Icook.to_csv(f'{food}Recipe\\{food}食譜.csv',encoding='utf-8-sig')
(Icook.style
...
plt.savefig(f"{food}Recipe\\{food}Material15.png", dpi = 300)
...
plt.savefig(f"{food}Recipe\\{food}Material_Pie.png", dpi = 300)
plt.show()
